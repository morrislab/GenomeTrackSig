...
write_sim_annotation <- function(simulation_name, sig_activities,
                                 sim_activity_file, sim_purity_file, sim_tumortype_file){
  # colnames from all signatures file
  header <- c("Cancer Type", colnames(read.delim(TrackSig.options()$signature_file)))
  header[2] <- "Sample Name"

  # binarize activity
  activity <- c("SIMULATED", simulation_name, rep(0, (length(header) - 2)))
  activity[header %in% unique(names(unlist(sig_activities)))] <- 1
  ...
  # write tumor type
  write.table(t(c(simulation_name, "SIMULATION")), file = sim_tumortype_file, append = T,
              col.names = F, row.names = F, quote = F, sep = "\t")
}
...
create_simulation_set <- function(with_CNA = FALSE, outdir = "simulation_results",
  ...
  write_sim_annotation(simulation_name, sig_activities,
                       sim_activity_file, sim_purity_file, sim_tumortype_file)
}
