...
detectActiveSigsCopy <- function(path, purity = 1,
  ...
  binCounts <- binningNmut(path, binSize)
...
# TrackSigCopy <- function(path,
#                          binSize,
#                          activeInSample,
#                          purity = 1,
#                          sampleID = NULL,
#                          referenceSignatures = readr::read_delim('~/Desktop/CBSP2021/TrackSigFreq-dev/COSMIC_v3.2_SBS_CRCh37.txt'),
#                          scoreMethod = "Signature",
#                          nCutoff = 10000,
#                          desiredMinSegLen = 1,
#                          refGenome = BSgenome.Hsapiens.UCSC.hg19::BSgenome.Hsapiens.UCSC.hg19) {
...
#   countsPerBin <- binCounts <- counts_subset <- NULL
#
#   # group mutations into bins of specified size
#   binCounts <- binningNmut(path, binSize)
#
#   # form counts matrix where each row is a mutation type and each column is a bin
#   counts_subset <- binCounts[,8:103]
