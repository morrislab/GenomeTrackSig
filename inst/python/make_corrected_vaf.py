...
def get_correct_vaf(cnv_regions, vcf_file, purity):
	...
	for record in variants:
		...
		output = output.append([[str(record.CHROM), str(record.POS), str(record.REF), str(record.ALT), str(sampled_ccf_1), str(sampled_ccf_2), str(alt_count), str(ref_count), str(purity), str(copy_number)]], ignore_index = True)
